...
class JLKCmdr(ptkcmd.PtkCmd):
    prompt = 'JLKCmdr > '
    intro = '''J-Link Commander v0.6
blank line for connection, ? for help
address and value use hexadecimal, count use decimal\n'''
    
    def __init__(self):
        super(JLKCmdr, self).__init__(self)
    ...
    def preloop(self):
        ...
        if os.path.isfile(self.dllpath):
            self.onecmd('')

    def emptyline(self):     
        try:
            self.jlk = jlink.JLink(self.dllpath, self.mcucore)

            print(f'CPU core is {self.jlk.read_core_type()}\n')
    ...
    def do_rd8(self, addr, cnt):
        ...
        arr = self.jlk.read_mem_U8(addr, cnt)
    ...
    def do_rd16(self, addr, cnt):
        ...
        arr = self.jlk.read_mem_U16(addr, cnt)
    ...
    def do_rd32(self, addr, cnt):
        ...
        arr = self.jlk.read_mem_U32(addr, cnt)
...
Syntax: savebin <filepath> <addr> <NumBytes>\n'''
        ...
        with open(file, 'wb') as f:
            data = self.jlk.read_mem_U8(addr, cnt)
...
Can only exec when Core halted\n'''
        ...
        if vals['XPSR'] & 0xFF == 3:
            if self.jlk.read_core_type() not in ['Cortex-M0', 'Cortex-M0+']:
                causes = hardfault.diagnosis(self.jlk)
                print("\n".join(causes))
            ...
            stackMem = self.jlk.read_mem_U32(fault_SP, 64)  # 读取个数须是8的整数倍
...
register field write: sv <peripheral>.<register>.<field> <dec>\n'''
        ...
        if val == None:
            ...
            values = self.jlk.read_mem_U32(addr, count)
            ...
            if isinstance(obj, svd.Register):
                ...
                value = self.jlk.read_U32(addr)
                value = value & (~obj.mask) | (val << obj.pos)
                self.jlk.write_U32(addr, value)
